...
def collect_all(config_path: str | None = None) -> tuple[dict[str, ClientStats], object]:
    ...
    wg_data = wireguard.collect()
    wg_networks = wireguard.client_networks(wg_data)
    data = merge_all(
        wg_data,
        conntrack.collect(cfg.blocklists, cfg.whitelist, wg_networks),
...
def scan(
    ...
    if not data:
        console.print('[yellow]No data collected. Check permissions and installed tools: conntrack, wg/awg, nft, xray logs.[/yellow]')
        return
...
@app.command()
def peers(config: str | None = typer.Option(None, '--config', '-c')):
    """Show WireGuard/AmneziaWG peer mapping: tunnel IP -> endpoint -> public key."""
    data, cfg = collect_all(config)
    table = Table(title='WireGuard/AmneziaWG peers')
    ...
    for c in sorted(data.values(), key=lambda x: x.ip):
        if not c.peer_public_key:
            continue
        table.add_row(
            c.ip,
            c.endpoint or '-',
            c.interface or '-',
            c.allowed_ips or '-',
            human_bytes(c.rx_bytes),
            human_bytes(c.tx_bytes),
            c.peer_public_key[:18] + '…',
        )
    console.print(table)
...
def report(
    ...
    for client in data.values():
        ...
        payload['clients'].append({
            ...
            'peer_public_key': client.peer_public_key,
