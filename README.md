...
## AmneziaWG/WireGuard traffic attribution update

This build detects both `wg` and `awg` tools and maps activity by tunnel IP instead of only the external endpoint IP.
...
vpn-guard scan
vpn-guard abuse-check
vpn-guard report -o /root/vpn-guard-report.json
```

For WireGuard/AmneziaWG the important columns are:

- `Client IP` — internal tunnel IP from peer AllowedIPs, for example `10.8.0.5`.
- `Endpoint` — real client endpoint IP:port.
- `Iface` — interface name, for example `wg0` or `awg0`.
- `blacklist_hits` / `Reason` — traffic to configured abuse networks.

If `abuse-check` shows a hit, find the same `Client IP` in `vpn-guard peers`; the public key there is the peer to disable.
